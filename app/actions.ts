...
async function findOrCreatePlayer(
  ...
  if (existingPlayers.length > 0) {
    ...
    if (historyPlayers.length > 0) {
      ...
      // Only update alliance if provided, but DON'T change current_name
      // This preserves the primary name while allowing aliases to work
      if (allianceId) {
        await sql`UPDATE players SET alliance_id = ${allianceId}, updated_at = NOW() WHERE id = ${playerId}`;
      }
    }
  ...
  if (!playerId) {
    ...
    // Ensure the current name is in history if it's new (only for exact current_name matches)
    const currentPlayer =
      await sql`SELECT current_name FROM players WHERE id = ${playerId}`;
    if (
      currentPlayer.length > 0 &&
      currentPlayer[0].current_name === playerName
    ) {
...
// Action to merge players (create aliases)
export async function mergePlayersAction(formData: FormData) {
  try {
    const primaryPlayerId = formData.get("primaryPlayerId") as string;
    const aliasPlayerId = formData.get("aliasPlayerId") as string;

    if (!primaryPlayerId || !aliasPlayerId) {
      return {
        success: false,
        error: "Both primary and alias player IDs are required",
      };
    }

    if (primaryPlayerId === aliasPlayerId) {
      return { success: false, error: "Cannot merge a player with themselves" };
    }

    // Get player names for logging
    const primaryPlayer =
      await sql`SELECT current_name FROM players WHERE id = ${primaryPlayerId}`;
    const aliasPlayer =
      await sql`SELECT current_name FROM players WHERE id = ${aliasPlayerId}`;

    if (primaryPlayer.length === 0 || aliasPlayer.length === 0) {
      return { success: false, error: "One or both players not found" };
    }

    // Transfer all daily_player_stats from alias player to primary player
    await sql`
      UPDATE daily_player_stats
      SET player_id = ${primaryPlayerId}
      WHERE player_id = ${aliasPlayerId}
      ON CONFLICT (player_id, event_phase_id) DO UPDATE SET
        alliance_ranking = COALESCE(EXCLUDED.alliance_ranking, daily_player_stats.alliance_ranking),
        power = COALESCE(EXCLUDED.power, daily_player_stats.power),
        player_rank = COALESCE(EXCLUDED.player_rank, daily_player_stats.player_rank),
        furnace_level = COALESCE(EXCLUDED.furnace_level, daily_player_stats.furnace_level),
        world_rank_placement = COALESCE(EXCLUDED.world_rank_placement, daily_player_stats.world_rank_placement),
        points = COALESCE(EXCLUDED.points, daily_player_stats.points),
        recorded_at = NOW()
    `;

    // Transfer name history from alias player to primary player
    await sql`
    ...
    // Add the alias player's current name to primary player's name history
    await sql`
      INSERT INTO player_name_history (player_id, name)
      VALUES (${primaryPlayerId}, ${aliasPlayer[0].current_name})
      ON CONFLICT (player_id, name) DO NOTHING
    `;

    // Delete the alias player record
    await sql`DELETE FROM players WHERE id = ${aliasPlayerId}`;

    return {
      success: true,
      message: `Successfully merged "${aliasPlayer[0].current_name}" into "${primaryPlayer[0].current_name}"`,
    };
  } catch (error: any) {
    console.error("Error merging players:", error);
    return {
      success: false,
      error: `Failed to merge players: ${error.message}`,
    };
  }
}
